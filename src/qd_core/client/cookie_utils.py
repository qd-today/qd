...
logger_cookiejar = Log("QD.Core.Client").getlogger()


def _debug(*args):
    if not get_settings().log.debug:
        return None
...
class CookieSession(RequestsCookieJar):
    def extract_cookies_to_jar(self, request: httpclient.HTTPRequest, response: httpclient.HTTPResponse):
        ...
        headers.getheaders = headers.get_list  # type: ignore
        res = MockResponse(headers)
        self.extract_cookies(res, req)  # type: ignore
