...
_UNSAFE_ATTRIBUTES = ["f_builtins", "f_globals", "f_locals", "gi_frame", "gi_code", "co_code", "func_globals"]
...
@timeout(get_settings().client_request.unsafe_eval_timeout)
def unsafe_eval(*args, **kwargs):
    return eval(*args, **kwargs)  # pylint: disable=eval-used


class BadCompilingInputError(Exception):
...
def check_for_pow(expr):
    ...
    """
    if "**" in expr:
        raise BadCompilingInputError("Power operation is not allowed")
...
def safe_eval(
    expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False, filename=None
):
    ...
    if not nocopy:
        ...
        if (globals_dict is not None and not isinstance(globals_dict, dict)) or (
...
datetime = WrapModule(
    __import__("datetime"), ["date", "datetime", "time", "timedelta", "timezone", "tzinfo", "MAXYEAR", "MINYEAR"]
)
dateutil = WrapModule(
    dateutil,
    {  # type: ignore
