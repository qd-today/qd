...
def password_hash(word, salt=None, iterations=400):
    if salt is None:
...
def aes_encrypt(
    word, key, iv=None, output="base64", padding=True, padding_style="pkcs7", mode=AES.MODE_CBC, no_packb=False
):
    if iv is None:
...
def aes_decrypt(
    word, key, iv=None, input="base64", padding=True, padding_style="pkcs7", mode=AES.MODE_CBC, no_packb=False
):
...
try:
    import passlib
    import passlib.hash
    from passlib.utils.handlers import (
        HasRawSalt,
        PrefixWrapper,
    )
...
class CryptHash(BaseHash):
    def __init__(self, algorithm):
        ...
        if not HAS_CRYPT:
            raise Exception(
                "crypt.crypt cannot be used as the 'crypt' python library is not installed or is unusable.",
                orig_exc=CRYPT_E,
            )
    ...
    def _salt(self, salt, salt_size):
        ...
        if (self.algo_data.salt_exact and len(ret) != self.algo_data.salt_size) or (
            not self.algo_data.salt_exact and len(ret) > self.algo_data.salt_size
        ):
...
class PasslibHash(BaseHash):
    ...
    def _clean_salt(self, salt):
        if not salt:
            return None
        if issubclass(
            self.crypt_algo.wrapped if isinstance(self.crypt_algo, PrefixWrapper) else self.crypt_algo, HasRawSalt
        ):
