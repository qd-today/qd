...
class UtilDelayParaHandler(BaseHandler):
    ...
    async def get(self):
        ...
        if seconds < 0:
            ...
            self.write(
                'Error, limited by delay_max_timeout, delay {seconds} second.'
            )
            return
        await gen.sleep(seconds)
                    ...
                    if await self.db.user.challenge_md5(
                        ...
                        if f.find('add') > -1:
                            ...
                            if notepadid == -1:
                                ...
                                raise Exception(f"记事本编号已存在, id_notepad: {notepadid}")
                ...
                for onnx_name in config.extra_onnx_name:
                    self.extra[onnx_name] = ddddocr.DdddOcr(
                        ...
                        import_onnx_path=os.path.join(
                            ...
                            "config", f"{onnx_name}.onnx"),
                        charsets_path=os.path.join(
                            os.path.abspath(
                                os.path.dirname(
                                    os.path.dirname(os.path.dirname(__file__)))),
                            "config", f"{onnx_name}.json"))
                    logger_web_util.info(
                        "成功加载自定义Onnx模型: %s.onnx", onnx_name)
    ...
    def slide_match(self, imgtarget: bytes, imgbg: bytes, comparison=False, simple_target=False):
        ...
        if not simple_target:
            try:
                ...
                logger_web_handler.debug("slide_match error: %s", e, exc_info=config.traceback_print)
    ...
    if img:
        ...
        if img.startswith("http"):
            try:
                ...
                logger_web_handler.debug("get_img_from_url error: %s", e, exc_info=config.traceback_print)
    ...
    async def get(self):
        ...
        try:
            if DDDDOCR_SERVER:
                ...
                rtv["Result"] = DDDDOCR_SERVER.slide_match(imgtarget, imgbg, comparison=comparison, simple_target=simple_target)
                rtv["状态"] = "OK"
            else:
                raise HTTPError(406)
        except Exception as e:
