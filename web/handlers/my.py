...
class MyHandler(BaseHandler):
    ...
    async def get(self):
        ...
        if (await self.db.user.get(user['id'], fields=('id',))):
            ...
            for tpl in await self.db.tpl.list(userid=user['id'], fields=('id', 'siteurl', 'sitename', 'banner', 'note', 'disabled', 'lock', 'last_success', 'ctime', 'mtime', 'fork', '_groups', 'updateable', 'tplurl'), limit=None):
                tplurl = tpl['tplurl']
            ...
            tasks = []
            for task in await self.db.task.list(user['id'], fields=('id', 'tplid', 'note', 'disabled', 'last_success', 'success_count', 'failed_count', 'last_failed', 'next', 'last_failed_count', 'ctime', '_groups'), limit=None):
...
class CheckUpdateHandler(BaseHandler):
    ...
    async def get(self):
        ...
        async with self.db.transaction() as sql_session:
            tpls = await self.db.tpl.list(userid=user['id'], fields=('id', 'siteurl', 'sitename', 'banner', 'note', 'disabled', 'lock', 'last_success', 'ctime', 'mtime', 'fork', 'tplurl', "updateable",), limit=None, sql_session=sql_session)
            
            tasks = []
            for task in await self.db.task.list(user['id'], fields=('id', 'tplid', 'note', 'disabled', 'last_success', 'success_count', 'failed_count', 'last_failed', 'next', 'last_failed_count', 'ctime', '_groups'), limit=None, sql_session=sql_session):
            ...
            tpls = await self.db.tpl.list(userid=user['id'], fields=('id', 'siteurl', 'sitename', 'banner', 'note', 'disabled', 'lock', 'last_success', 'ctime', 'mtime', 'fork', 'tplurl', "updateable", '_groups'), limit=None, sql_session=sql_session)
